...
const BlogCard = ({ blog, isReversed }) => {
  ...
  console.log("BlogCard rendered with blog:", blog.thumbnail);
  return (
    <div
      className={` text-white flex py-4  flex-row border-2 rounded px-4 mb-4 bg-white/10 ${
    ...
    >
      ...
      <Link
        className="md:w-5/12 w-full flex justify-center hover:scale-105 transition-transform duration-300"
        href={`/blog/${blog.id}`}
      >
        <img
          src={
      ...
      {/* Text content */}
      <div
        className={`md:w-7/12 w-full flex flex-col justify-between ${
          isReversed ? "pr-4 mr-4" : "pl-4 ml-4"
        } relative`}
      >
        <div>
